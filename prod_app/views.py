...
from prod_app.models import uo, ativo, uep, poco
import json
...
def verificar_superuser_admin_comum(user): # Verifica se o usuário possui permissão como superusuário, administrador ou comum
    return (
        user.is_superuser
        or user.groups.filter(name="Administrador").exists()
        or user.groups.filter(name="Comum").exists()
    )


def verificar_superuser_ou_admin(user): # Verifica se o usuário possui permissão como superusuário ou administrador
    return user.is_superuser or user.groups.filter(name="Administrador").exists()


@user_passes_test(verificar_superuser_admin_comum)
@login_required()
def home(request):
...
def ajax_adicionar_uo(request): # Adiciona uma nova UO via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.add_uo") == True:
        autorizacao = True
    else:
        autorizacao = False
...
def ajax_editar_uo(request): # Edita os dados de uma UO via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.edit_uo") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_excluir_uo(request): # Exclui uma UO existente via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.delete_uo") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_adicionar_ativo(request): # Adiciona um novo ativo via AJAX.
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.add_ativo") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_editar_ativo(request): # Edita os dados de um ativo via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.edit_ativo") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_excluir_ativo(request): # Exclui um ativo existente via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.delete_ativo") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_adicionar_uep(request): # Adiciona uma nova UEP via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.add_uep") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_editar_uep(request): # Edita os dados de uma UEP via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.edit_uep") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_excluir_uep(request): # Exclui uma UEP via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.delete_uep") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_adicionar_poco(request): # Adiciona um novo poço via AJAX.
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.add_poco") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_editar_poco(request): # Edita os dados de um poço via AJAX
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.edit_poco") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
...
def ajax_excluir_poco(request): # Exclui um poço existente via AJAX
    ...
    autorizacao = False
    sucesso = False
    if request.user.has_perm("gideao_project.prod.delete_poco") == True:
        autorizacao = True
    else:
        autorizacao = False
    if request.method == "POST" and autorizacao == True:
