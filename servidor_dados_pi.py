...
class MyTCPHandler(SocketServer.BaseRequestHandler):
    def handle(self):
        ...
        metodo_corr = argumentos[0]
        nome_servidor = argumentos[1]
        tag_corr = argumentos[2]
        inicio_corr = argumentos[3]
        fim_corr = argumentos[4]
        span_corr = argumentos[5]

        if not tag_corr or not nome_servidor:
            return

        conexao_corr = servidores[nome_servidor]
        estadosDiscretos = eval(argumentos[6])
        if metodo_corr == "Recorded":
        ...
        elif metodo_corr == "Interpolated":
            dfgp = gp.getValoresInterpolados(
                conexao_corr,
                tag_corr,
                inicio_corr,
                fim_corr,
