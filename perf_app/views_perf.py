...
from perf_app.services.rtomongoservice import get_rto_data

import django
from perf_app.models import ativo_perf, poco_perf, uo_perf
...
from perf_app.models import amostra_perf, analise_perf, servidor_perf, sonda_perf

from django.core import serializers
from django.http import HttpResponse

def superuser_or_admin_or_comum(user):
...
def ajax_adicionar_instancia_amostras_especialista_perf(request):
    if request.headers.get("x-requested-with") == "XMLHttpRequest" and request.POST:
        if request.POST:
            ...
            try:
                sonda = None
                for row in sonda_perf.objects.raw("select * from perf_sonda_perf"):  ## monitor_app_sonda_perf para  perf_sonda_perf
